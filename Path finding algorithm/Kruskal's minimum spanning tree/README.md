...
## Graph used and the the Minimum Spanning Tree made in this code
...
int get_parent(int vertex) {
	if (vertex == connection_table[vertex])
		return vertex;
	return connection_table[vertex] = get_parent(connection_table[vertex]); //  (connection_table[vertex] =) << 없어도 됨
}
