...
import heapq
# Push gates into pq.
# (repeat)Pop from pq and then update and push nodes connected to the popped node.
# Find min summit from summits
def solution(n, paths, gates, summits):
    ...
    while len(pq):
        ...
        for next_node in adjacencies[node]:
            if already_shortests[next_node[0]] or node in summits:
                continue
            new_intensity=max(intensity,next_node[1])
            if new_intensity<=intensity_table[next_node[0]]:
                intensity_table[next_node[0]]=new_intensity
                heapq.heappush(pq,(new_intensity,next_node[0]))
